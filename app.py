...
def check_face_during_registration():
    ...
    try:
        ...
        if face_rgb.size == 0:
            return jsonify({"status": "no_face", "message": "Face region empty"})
        ...
        try:
            ...
            return jsonify({"status": "error", "message": f"Failed to process face: {str(e)}"})
        ...
        # If distance is below threshold, face is already registered
